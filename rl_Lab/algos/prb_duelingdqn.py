...
class DuelingDQNPrb(Algo):
    def __init__(self, obs_dim, obs_shape, state_dim, action_dim, cfg, device) -> None:
        ...
        if len(self.obs_shape) == 1:
            ...
            self.replay = PrioritizedReplayBuffer(capacity= cfg["buffer_size"],obs_shape= self.obs_shape, store_ep_info = True)
        else:
            ...
            self.replay = PrioritizedReplayBuffer(capacity= cfg["buffer_size"],obs_shape= self.obs_shape, store_uint8 = True,  store_ep_info = True)
        ...
        # Priority sampling
        self.sample_alpha= cfg['sample_alpha']
        self.sample_beta_start= cfg['sample_beta_start']
        self.sample_beta_end = cfg['sample_beta_end']
        self.sample_eps = cfg['sample_eps']
    ...
    def select_action(self, obs_np: np.ndarray, eval_mode: bool = False):
        ...
        q = DuelingDQNPrb.combine_dueling(value,adv)
    ...
    def update(self, global_step) -> dict[str, float] | None:
        ...
        if self.replay.can_sample(batch_size=self.batch_size):
            ...
            idxs = sample['idxs']
            P = torch.as_tensor(sample['prob'], device=self.device).clamp_min(1e-12).unsqueeze(1)
            N = float(len(self.replay))
            
            # before sampling
            t = float(global_step)
            T = float(self.cfg.get('beta_anneal_steps', 1_000_000))   # or total train steps
            frac = min(1.0, t / T)
            beta = self.sample_beta_start + frac * (self.sample_beta_end - self.sample_beta_start)
            beta = max(0.0, min(1.0, beta))
            
            w_is = (1.0/(N*P)).pow(beta)
            w_is = w_is/w_is.max()
            ...
            with torch.no_grad():
                ...
                next_q_online = DuelingDQNPrb.combine_dueling(next_q_online_val,next_q_online_adv) 
                ...
                next_q_target = DuelingDQNPrb.combine_dueling(next_q_target_val, next_q_target_adv)
            ...
            q_tensor = DuelingDQNPrb.combine_dueling(q_val, q_adv)
            q_values = q_tensor.gather(1, action)
            loss = self.loss_fn(q_values, target_q)
            loss = (w_is * loss).mean()

            with torch.no_grad():
                td_error = (q_values-target_q).abs()
                new_p = (td_error+ self.sample_eps).pow(self.sample_alpha).squeeze(1)

            self.replay.update_prob(idxs=idxs, new_priorities= new_p.detach().cpu().numpy())
            ...
            metrics = {
                'loss': mean_loss,
                'td_abs_mean': float(td_error.mean().item()),
                'priority_max': float(new_p.max().item()),
            }
            return metrics
        return None
